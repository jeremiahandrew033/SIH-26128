...
def test_upload_attachment_validation(client, authed_vet_client):
    ...
    hr_res = authed_vet_client.post("/api/v1/health-reports", json={
        "farmer_id": farmer_id,
        "animal_id": animal_id,
        "report_type": "illness"
    })
    ...
    res = authed_vet_client.post(f"/api/v1/health-reports/{case_id}/attachments", files=files)
    assert res.status_code == 201
    data = res.json()
    assert data["case_id"] == case_id
    assert "/uploads/" in data["file_path"]

def test_invalid_file_type_rejection(client, authed_vet_client):
    fake_txt = io.BytesIO(b"Hello text file")
    files = {"file": ("malicious.exe", fake_txt, "application/octet-stream")}

    res = authed_vet_client.post("/api/v1/health-reports/LIV-TEST-0001/attachments", files=files)
